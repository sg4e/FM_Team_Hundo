...
2. An OBS-friendly widget for team stats is available for anyone to use:
   
   1. Go to the team page whose stats you're interested in
   
   2. Right-click `OBS Stats Widget`, then "Copy Link"
   
   3. Create a Browser Source in OBS
   
   4. Paste the link as the URL of the Browser Source
   
   5. (Optional:) You can customize which stats to display and enable the dark-mode layout by editing the `true`/`false` values in the URL
